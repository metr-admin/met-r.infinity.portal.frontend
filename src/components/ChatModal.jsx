...
const ChatModal = ({ isOpen, onClose, domain = 'general' }) => {
  ...
  const processSearchResponse = async (response) => {
    ...
    if (docsStartIdx !== -1 && docsEndIdx !== -1) {
      markdown = fullText.substring(0, docsStartIdx).trim();
      ...
      try {
        ...
        console.error('Parse error:', e, 'JSON:', docsJson);
      }
    }
                              ...
                              {expandedDocs[idx] && (
                                <div className="px-3 pb-3 space-y-2">
                                  {msg.supportingDocs.map((doc, docIdx) => {
                                    ...
                                    const docTitle = doc.title || doc.metadata?.title || 'Untitled Document';
                                    const docLink = doc.link || doc.metadata?.link;
                                    ...
                                    return (
                                      <div key={docIdx} className="bg-white rounded-lg p-3 border border-accent-blue/20 hover:border-accent-blue/40 transition-all">
                                        ...
                                        <div className="flex items-center justify-between">
                                          ...
                                          {doc.filepath && (
                                            <button
                                              onClick={() => handleDocumentView(doc.filepath)}
                                              className="text-xs text-accent-blue hover:text-accent-purple font-medium flex items-center gap-1"
                                            >
                                              <span>View</span>
                          ...
                          {msg.message_id && (
                            <div className="flex gap-2 px-2">
                              <button
                                ...
                                className={`p-1 rounded hover:bg-gray-200 transition-colors ${msg.feedback === true ? 'text-green-600' : 'text-gray-400'
                                  }`}
                                title="Like"
                              >
                              ...
                              <button
                                ...
                                className={`p-1 rounded hover:bg-gray-200 transition-colors ${msg.feedback === false ? 'text-red-600' : 'text-gray-400'
                                  }`}
                                title="Dislike"
                              >
