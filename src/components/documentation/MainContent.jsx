...
const MainContent = ({ currentDoc, allDocs, onDocSelect, hierarchicalDocs }) => {
  ...
  const navigationUtils = React.useMemo(() => createNavigationUtils(), []);
  ...
  // Extract headings using configurable processor
  const extractHeadings = (htmlContent) => {
    return contentProcessor.extractHeadings(htmlContent);
  };
  ...
  useEffect(() => {
    ...
    const content = contentProcessor.processContent(currentDoc.attributes?.htmlContent || currentDoc.attributes?.content || currentDoc.attributes?.bodyContent);
  ...
  // Handle clicks on internal document links using configurable navigation
  const handleContentClick = (e) => {
    ...
    if (target) {
      ...
      const { targetDoc, anchor } = navigationUtils.findDocument(linkHref, allDocs, hierarchicalDocs);
      navigationUtils.navigateToDocument(targetDoc, anchor, onDocSelect);
    }
  };
  // Use configurable content processor
  const processContent = (htmlContent) => {
    return contentProcessor.processContent(htmlContent);
  };
  ...
  const processedContent = contentProcessor.processContent(attributes?.htmlContent || attributes?.content || attributes?.bodyContent);
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 min-w-0 overflow-x-auto">
        ...
        <div className="mb-4 sm:mb-6">
          <nav className="flex items-center space-x-2 text-xs sm:text-sm text-gray-500 overflow-x-auto">
            {navigationUtils.generateBreadcrumb(
              branchName,
        ...
        <div className="w-full max-w-none lg:max-w-4xl overflow-x-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mt-8 sm:mt-12 pt-6 sm:pt-8 border-t border-gray-200">
            {(() => {
              ...
              const { prevDoc, nextDoc } = navigationUtils.getNavigationContext(currentDoc, hierarchicalDocs);
